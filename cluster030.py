...
if __name__ == "__main__":
    ...
    for burst_error_rate in burst_error_rates:
        ...
        simulation.simulation_results_to_csv(data_dictionary, '030_results_expanded_0712')
    
    
    # data_dictionary = pd.read_csv('030_results_expanded_0712.csv')
    ...
    #         plt.scatter(rounds[int(len(rounds)/2):], logical_error_rates[int(len(rounds)/2):], label='distance = ' + str(distance) + ', phenomenological noise = 3.0%, \nburst error rate = ' + str(burst_error_rate * 100) +'%')
    #         plt.scatter(rounds[:int(len(rounds)/2)], logical_error_rates[:int(len(rounds)/2)], label='distance = ' + str(distance) + ', phenomenological noise = 3.0%')
    ...
    #     plt.plot(np.linspace(0.09, 0.16, 300), res.intercept + res.slope*np.linspace(0.09, 0.16, 300), c=color)
    ...
    # plt.minorticks_on()
    # plt.savefig(str(noises[0] * 100) + '%_phenomenological_noise_burst_error_threshold_new_0712.png', bbox_inches="tight")
    # plt.clf()
