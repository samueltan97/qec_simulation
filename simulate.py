...
def simulate_logical_error_rate(num_shots: int, rounds:List[int], distances: List[int], noises:List[float], \
    ...
    total_simulations = len(rounds) * len(distances) * len(noises)
    ...
    results = np.zeros(shape=(len(distances), len(noises), len(rounds)))
    for d_index, distance in enumerate(distances):
        for n_index, noise in enumerate(noises):
            for r_index, round in enumerate(rounds):
                ...
                results[d_index][n_index][r_index] = decoding.count_logical_errors(circuit, num_shots)/num_shots    
                completed_simulations += 1
                print('Completed: ' + str(completed_simulations)  + '/' + str(total_simulations) )       
    return results

def plot_simulation_results(simulation_results:np.ndarray, x_axis:str, x_axis_data:List[float], graph_label:str, graph_label_data:List[float], x_label:Optional[str]='' \
