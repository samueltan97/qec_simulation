...
if __name__ == "__main__":
    ...
    for burst_error_rate in burst_error_rates:
        ...
        simulation.simulation_results_to_csv(data_dictionary, '020_results_expanded_0705')
    
    
    data_dictionary = pd.read_csv('020_results_expanded_0705.csv')
    ...
    for d_index, distance in enumerate(distances):
        ...
        for burst_error_rate in burst_error_rates:
            ...
            plt.scatter(rounds[int(len(rounds)/2):], logical_error_rates[int(len(rounds)/2):], label='distance = ' + str(distance) + ', phenomenological noise = 2.0%, \nburst error rate = ' + str(burst_error_rate * 100) +'%')
            plt.scatter(rounds[:int(len(rounds)/2)], logical_error_rates[:int(len(rounds)/2)], label='distance = ' + str(distance) + ', phenomenological noise = 2.0%')
